...
2. The spec sections the ticket names, `docs/adr/*.md`, `AGENTS.md`, the Coding rules in `~/.agents/AGENTS.md`, and `~/.agents/docs/agents/code-review.md`.
...
It reports every issue it finds, including uncertain and low-severity ones, because its job is coverage and this session does the filtering. This session then drops and grades findings by the Reviewing rules. Each surviving finding quotes the hunk and states the failing input or state and the wrong result.
...
Merge both passes into one list, each finding in the shape the Reviewing rules give. Scope creep is `medium` and `fix` unless the ticket is the only place the change can live. Deferred items name the ticket or follow-up that should own them.

Send it to `<author-name>` with SendMessage. First line `Review round <k> for PR #<pr>`, then the criteria table, the findings, the validate exit code, and the verdict line from the Reviewing rules last. If the send fails, run ListAgents and use the row named `author-<n>`.
...
After the third verdict, or after `verdict: agree`, stop and wait for messages.
